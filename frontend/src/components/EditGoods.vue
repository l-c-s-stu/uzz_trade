    ...
    <div class="container">
      ...
      <div v-else-if="error" class="error-card">
        ...
        <div class="error-actions">
          <button @click="loadGoodsInfo" class="btn btn-primary">重试</button>
          <router-link to="/" class="btn btn-outline">返回首页</router-link>
        </div>
      </div>

      <!-- 未登录提示 -->
      <div v-else-if="!isLoggedIn" class="error-card">
        <h3>请先登录</h3>
        <p>编辑商品需要先登录账号</p>
      ...
      <!-- 权限不足 -->
      <div v-else-if="goods && !goods.is_owner" class="error-card">
        <h3>权限不足</h3>
        <p>您只能编辑自己发布的商品</p>
      ...
      <!-- 编辑表单 -->
      <div v-else-if="goods" class="form-card">
        <div class="page-header">
          <h1 class="section-title">编辑商品</h1>
        </div>
        ...
        <form @submit.prevent="handleSubmit" v-if="!success">
          ...
          <!-- 当前图片 -->
          <div v-if="goods.image" class="form-group">
            <label>当前主图</label>
          ...
          <div class="form-group">
            ...
            <input
              ...
              :disabled="loading"
            />
            <small>支持 JPG、PNG、GIF 格式。不选择文件则保持原图片</small>
        ...
        <!-- 成功后的操作 -->
        <div v-if="success && updatedGoods" class="success-actions">
          <h3>🎉 商品修改成功！</h3>
          <p>商品《{{ updatedGoods.title }}》已成功修改</p>
          <div class="action-buttons">
            <router-link
              :to="`/goods/${updatedGoods.id}`"
...
const loadGoodsInfo = async () => {
  ...
  try {
    ...
    formData.value = {
      ...
      status: goods.value.status || 1,
      category_id: goods.value.category?.id || null,
      image: null
    }
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const response = await goodsAPI.updateGood(goodsId.value, submitData)
    
    if (response.data) {
      updatedGoods.value = response.data
      success.value = true
      successMessage.value = '商品修改成功！'
    } else {
    ...
    if (err.response?.data) {
      ...
      if (typeof errorData === 'object') {
        ...
        errorMessage.value = errorMessages.join('; ') || '修改失败，请检查输入信息'
      } else {
        errorMessage.value = errorData || '修改失败，请检查输入信息'
      }
    } else if (err.response?.status === 401) {
      errorMessage.value = '请先登录'
      router.push('/login')
    } else if (err.response?.status === 403) {
      errorMessage.value = '无权修改此商品'
    } else {
      errorMessage.value = '修改失败：网络错误'
    }
  } finally {
    loading.value = false
  }
